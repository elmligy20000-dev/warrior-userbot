...
GROUP_DELAY = 20 # 5 ثواني بين كل جروب زي ما طلبت
MESSAGE_DELAY = 50 # 3 ثواني بين الرسالتين
...
def gen_code(days=30):
    code = ''.join(random.choices('ABCDEKLMNPQXYZ6789', k=7))
    db['codes'][code] = days
    save_db()
    return code
...
async def callback(event):
    ...
    # ادارة الحساب
        ...
        btns = [
            [Button.inline("تسجيل السيشن", b"login_session")],
            [Button.inline("رجوع", b"back_main")]
        ]
        await event.edit(f"{PC}<b>اضغط تسجيل السيشن</b>", buttons=btns, parse_mode='html')
    ...
    # تجربة مجانية
        ...
        acc = get_account(uid)
        if not acc:
            text = f"{USER} <b>اضغط لـ تسجيل الحساب اولاً:</b>"
            btns = [
        ...
        if not groups:
            ...
            if len(groups) > 100:
                groups_text += f"\n{SIGNAL}... و {len(groups)-100} جروب اخر\n{SIGNAL}استخدم 'حذف برقم' للباقي"

        btns = [
            [Button.inline(f"اضافة يدوي", b"add_group"), Button.inline(f"حذف برقم", b"del_group_num")],
        ...
        # الفاصل الزمني
        mn = acc.get('interval_min', 180)
        mx = acc.get('interval_max', 400)
        if mn == mx:
    ...
    if action == 'code':
        ...
        try:
            await bot.send_message(
                ...
                f"{BOLT} تواصل مع المبرمج لتجديد الاشتراك\n"
        ...
        idx = int(action[1:]) - 1 # msg1 -> 0, msg2 -> 1

        if idx < 0 or idx > 1: # حماية - اكتر من 2
            await event.reply(f"{BOLT}الحد الاقصى 2 رسايل بس", parse_mode='html')
            del waiting_for[uid]
            return
        ...
        try:
            ...
            if interval < 180:
                await event.reply(
                    ...
                    f"{SIGNAL} اقل حاجة 180 ثانية عشان متاخدش فلود\n"
